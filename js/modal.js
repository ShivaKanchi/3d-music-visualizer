  ...
  fetch("data/music-metadata.json")
    ...
    .then((data) => {
      ...
      entries.forEach((entry) => {
        ...
        li.addEventListener("click", () => {
          closeSongPopup();
          if (li.dataset.file) {
            window.currentVisualization = li.dataset.viz || "spiral";
            if (window.loadAudioFromUrl) {
              window.loadAudioFromUrl("music/" + li.dataset.file);
            }
          }
        });
        ul.appendChild(li);
    ...
    .catch(() => {
      ...
      fallback.forEach((entry) => {
        ...
        li.addEventListener("click", () => {
          closeSongPopup();
          window.currentVisualization = entry.viz;
          if (window.loadAudioFromUrl) {
            window.loadAudioFromUrl("music/" + entry.file);
          }
        });
        ul.appendChild(li);
      });
